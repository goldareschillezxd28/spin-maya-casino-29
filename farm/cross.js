...
function say(text,mood){calloutEl.textContent=text;calloutEl.classList.remove("coop-callout-glad","coop-callout-sad");if(mood==="glad"){calloutEl.classList.add("coop-callout-glad-53efff6a");}else if(mood==="sad"){calloutEl.classList.add("coop-callout-sad-53efff6a");}}
function showBanner(text,won){bannerEl.textContent=text;bannerEl.className="coop-banner-53efff6a"+
(won?"coop-banner-win":"coop-banner-lose");bannerEl.hidden=false;}
function hideBanner(){bannerEl.hidden=true;bannerEl.textContent="";}
function buildRoad(){stripEl.innerHTML="";dotsEl.innerHTML="";laneTiles=[];dotEls=[];curbEl=document.createElement("div");curbEl.className="coop-lane-53efff6a coop-curb-53efff6a";var curbTag=document.createElement("span");curbTag.className="coop-lane-mult-53efff6a";curbTag.textContent="Start";curbEl.appendChild(curbTag);stripEl.appendChild(curbEl);for(var i=0;i<LANE_TOTAL;i+=1){var tile=document.createElement("div");tile.className="coop-lane-53efff6a";var tag=document.createElement("span");tag.className="coop-lane-mult-53efff6a";tag.textContent=LADDER[i].toFixed(2)+"×";tile.appendChild(tag);stripEl.appendChild(tile);laneTiles.push(tile);var dot=document.createElement("li");dot.className="coop-dot-53efff6a";dotsEl.appendChild(dot);dotEls.push(dot);}
henEl=document.createElement("span");henEl.className="coop-hen-53efff6a";henEl.setAttribute("aria-hidden","true");henEl.textContent="🐔";curbEl.appendChild(henEl);}
function clearRoad(){for(var i=0;i<LANE_TOTAL;i+=1){var tile=laneTiles[i];tile.classList.remove("coop-lane-clear","coop-lane-hit");var extras=tile.querySelectorAll(".coop-car-53efff6a, .coop-feather-53efff6a");for(var k=0;k<extras.length;k+=1){tile.removeChild(extras[k]);}
dotEls[i].classList.remove("coop-dot-done-53efff6a");}
...
attemptHop();}
function attemptHop(){run.busy=true;setMoves();var target=run.lane;var tile=laneTiles[target];tile.appendChild(henEl);henEl.classList.remove("coop-hen-hop-53efff6a");void henEl.offsetWidth;henEl.classList.add("coop-hen-hop-53efff6a");centerTile(tile);var lucky=Math.random()<surviveOdds(target);window.setTimeout(function(){resolveHop(target,lucky);},HOP_MS);}
function resolveHop(target,lucky){if(!lucky){crash(target);return;}
run.lane=target+1;laneTiles[target].classList.add("coop-lane-clear-53efff6a");paintDots();paintGauges();if(run.lane===LANE_TOTAL){settle(true,true);return;}
run.busy=false;setMoves();say("Lane "+run.lane+" cleared — "+
multAt(run.lane).toFixed(2)+"× is yours if you stop now.","glad");}
function crash(target){var tile=laneTiles[target];tile.classList.add("coop-lane-hit-53efff6a");var car=document.createElement("span");car.className="coop-car-53efff6a";car.setAttribute("aria-hidden","true");car.textContent=TRAFFIC[Math.floor(Math.random()*TRAFFIC.length)];tile.appendChild(car);henEl.classList.add("coop-hen-bonk-53efff6a");for(var i=0;i<3;i+=1){var feather=document.createElement("span");feather.className="coop-feather-53efff6a";feather.setAttribute("aria-hidden","true");feather.style.left=(18+i*18)+"px";feather.style.animationDelay=(i*0.12)+"s";tile.appendChild(feather);}
